...
var Concentration = Concentration || (function() {
    ...
    const styles = {
        ...
        addConcentration = (token, playerid, spell) => {
            ...
            if(getAttrByName(character.id,'npc_name')===undefined && ((token.get('controlledby').split(',').includes(playerid) || token.get('controlledby').split(',').includes('all')) ||
        ...
        sendConfigMenu = (first, message) => {
            ...
            let markerButton = makeButton(state[state_name].config.statusmarker, '!' + state[state_name].config.command + ' config statusmarker|'+markerDropdown, styles.button + styles.float.right),
                ...
                updateCharnameButton = makeButton('Sleepydude','!' + state[state_name].config.command + ' update_charname', styles.button + styles.float.right),
                ...
                listItems = [
                    ...
                    '<span style="'+styles.float.left+'">Update Charname:</span> ' + updateCharnameButton,
        ...
        update_charname = () => {
            let characters = findObjs({ _type: 'attribute', name: 'charname_output' })

            characters.forEach(character => {
                if(character.get('current')==0){
                    character.set('current','{{charname=@{npc_name}}}')
                    log('Updated NPC! id='+character.get('_id'))
                }
            });
        },
        ...
        setDefaults = (reset) => {
            ...
            if(!state[state_name].config){
                ...
                if(!state[state_name].config.hasOwnProperty('update_charname')){
                    state[state_name].config.update_charname = defaults.config.update_charname;
                }
            }
